package com.yuzukiku.dockerquest.aop;
...
public class KafkaOperationsSendingAdvice {
    ...
    public void logReturn(JoinPoint joinPoint) {
        try {
            logger.debug("Kafka send invoked with args: {}", objectMapper.writeValueAsString(joinPoint.getArgs()));
        } catch (JsonProcessingException e) {
            logger.error("Failed to serialize Kafka send result: {}", e.getMessage());
        }
    }
}
